package com.programmerbaper.skripsi.misc;

public class Config {

    public static final String MY_PREFERENCES = "MY_PREF";
    public static final String ID_USER = "ID_USER";
    public static final String ID_PEMILIK = "ID_PEMILIK";
    public static final String USERNAME = "USERNAME";
    public static final String PASSWORD = "PASSWORD";
    public static final String DATA_TRANSAKSI = "DATA_TRANSAKSI";
    public static final String FCM_TOKEN = "FCM_TOKEN";


    public static final String BASE_URL_OWM = "http://api.openweathermap.org/data/2.5/";
    public static final String API_KEY_OWM = "f70a95166eddb51376e93d2fe00e922b";

    public static final String BASE_URL = "http://www.pedagangkeliling.com/";
    public static final String BASE_URL_API = "http://www.pedagangkeliling.com/api/pedagang/";

    public static final String BASE_URL_DM = "https://maps.googleapis.com/maps/api/distancematrix/";

}
